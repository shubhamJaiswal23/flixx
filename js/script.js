const global = {
  ...
  api: {
    apiKey: 'eee71ae80270eeba683f78b87b2cfa7c',
...
//Diplay Backdrop on Details Page
...
function displayPagination() {
  ...
  <button class="btn btn-primary" id="prev">Prev</button>
          ...
          <div class="page-counter">Page ${global.search.pages} of ${global.search.totalPages}</div>
  `;
  ...
  if (global.search.pages === global.search.totalPages) {
...
async function searchAPIData() {
  ...
  const API_URL = global.api.apiUrl;
  const response = await fetch(
    `${API_URL}search/${global.search.type}?api_key=${API_KEY}&language=en-US&query=${global.search.term}&page=${global.search.pages}`
  );
...
//Show and hide the Spinner while fetching the Data
